      ...
      markets.map(async (market) => {
        try {
          ...
          return {
            ...
            country: onchain.sourceCountry || market.country,
            ...
            news_items: market.news_items
              ? {
                  ...market.news_items,
                  headline: onchain.newsHeadline || market.news_items.headline,
                  url: onchain.newsUrl || market.news_items.url,
                  country: onchain.sourceCountry || market.news_items.country,
                }
              : market.news_items,
            ...
            initial_probability_yes: Number(
              onchain.initialProbabilityYes ?? market.initial_probability_yes ?? 50,
            ),
            userBets: userBets
              ? {
                  yes: formatUnits(userBets.yes, 6),
                  no: formatUnits(userBets.no, 6),
                }
              : undefined,
            userEURCBets: userEURCBets
              ? {
                  yes: formatUnits(userEURCBets.yes, 6),
                  no: formatUnits(userEURCBets.no, 6),
                }
              : undefined,
          };
        } catch {
